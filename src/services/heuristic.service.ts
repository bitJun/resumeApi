...
function getBasicInfo(text: string): BasicInfo {
  const email = text.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i)?.[0] ?? "";
  const phone =
    text.match(/(?:\+?\d{1,3}[-\s]?)?(?:1[3-9]\d{9}|\d{3}[-\s]?\d{3}[-\s]?\d{4})/)?.[0] ?? "";
  const city =
    CITY_LIBRARY.find((item) => new RegExp(item.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i").test(text)) ?? "";
...
function collectSection(text: string, pattern: RegExp) {
...
function inferDegree(block: string) {
  const match =
    block.match(/博士|Ph\.?D|硕士|Master|本科|Bachelor|大专|Associate|高中/i)?.[0] ?? "";
  return match;
}

function inferGraduationTime(block: string) {
  return block.match(/\b(20\d{2}|19\d{2})(?:[./-](0?[1-9]|1[0-2]))?\b/g)?.join(" - ") ?? "";
}

function getEducations(text: string): EducationItem[] {
  ...
  return blocks
    .map((block) => {
      ...
      const major =
        lines.find((line) => /(专业|major|computer|software|data|design|engineering|金融|数学|统计)/i.test(line)) ??
        "";
...
function getExperiences(text: string): ExperienceItem[] {
  ...
  return blocks
    .map((block) => {
      ...
      const companyName =
        lines.find((line) => /(公司|科技|集团|有限公司|Inc|Ltd|LLC|Corp|Studio|科技|网络|University)/i.test(line)) ??
        headline.split(/[-|｜·]/)[0] ??
        headline;
      const title =
        lines.find((line) => /(工程师|经理|开发|设计|运营|分析师|Architect|Engineer|Manager|Developer|Designer|Lead)/i.test(line)) ??
        headline.split(/[-|｜·]/)[1] ??
        "";
...
function getProjects(text: string): ProjectItem[] {
...
function getSkills(text: string) {
  ...
  const directSectionSkills = uniqueStrings(
    section
      .split("\n")
...
export function extractResumeHeuristically(text: string): ResumeExtraction {
  return {
