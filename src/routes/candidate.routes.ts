...
import {
  ...
  updateCandidateExtractionStatus,
...
import { CandidateStatus, ResumeExtraction } from "../types/index.js";
...
function serializeCandidate(candidate: ReturnType<typeof getCandidateById>) {
  if (!candidate) {
    return null;
  }
...
router.get("/", (req, res) => {
  ...
  if (skill) {
    items = items.filter((candidate) =>
      candidate.extraction.skills.some((item) => item.toLowerCase().includes(skill))
    );
  }
...
router.patch("/:id/extraction", (req, res) => {
  ...
  const payload = req.body as Partial<ResumeExtraction>;
  const mergedExtraction: ResumeExtraction = {
    basicInfo: {
...
router.patch("/:id/status", (req, res) => {
  const status = req.body.status as CandidateStatus | undefined;
