...
function calculateSkillScore(candidateSkills, job) {
...
function calculateExperienceScore(extraction, job) {
    ...
    const experienceText = normalizeKeyword(extraction.experiences.map((item) => `${item.title} ${item.summary}`).join(" "));
    const roleKeywordMatches = job.mustSkills.filter((skill) => experienceText.includes(normalizeKeyword(skill))).length;
    const titleMatches = /(engineer|developer|architect|manager|lead|分析|开发|工程师|负责人|经理)/i.test(extraction.experiences.map((item) => item.title).join(" "));
...
function calculateEducationScore(extraction, job) {
    const text = `${job.title} ${job.description}`.toLowerCase();
    const highestDegree = extraction.educations.find((item) => /博士|phd/i.test(item.degree))?.degree ??
...
function buildComment(extraction, job, scores) {
    ...
    return `${name} 与「${job.title}」岗位的综合匹配度为 ${scores.overallScore} 分。${strengths.join("；")}。${risks.join("；")}。`.replace(/。+/g, "。");
}
export async function scoreCandidateAgainstJob(candidate, job) {
