...
async function requestOpenAI(systemPrompt, userPrompt) {
    if (!env.openAiApiKey) {
        return null;
    }
...
export async function extractResumeWithAI(text, heuristicResult, originalName) {
...
export async function scoreResumeWithAI(extraction, job, heuristicResult) {
    const content = await requestOpenAI([
