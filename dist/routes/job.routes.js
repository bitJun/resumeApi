...
import { getCandidateById, getJobDescriptionById, listCandidates, listCandidateMatchesByJob, listJobDescriptions, upsertCandidateMatch, upsertJobDescription } from "../db/index.js";
...
router.post("/:id/score-candidates", async (req, res, next) => {
    try {
        ...
        const candidates = candidateIds.length
            ? candidateIds.map((id) => getCandidateById(String(id))).filter(Boolean)
            : listCandidates();
        const matches = [];
